...
function Signup() {
    ...
    const signup = async () => {
        try {
            const signupRes = axios.post("/api/user/signup", { name: name, email: email, password: password })

            if ((await signupRes).data.status == "success") {
                alert("signup success");

                const loginRes = axios.post("/api/user/login", { email: email, password: password })
