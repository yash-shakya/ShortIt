...
function Signup() {
    ...
    const signup = async () => {
        try {
            const signupRes = axios.post("http://localhost:4000/user/signup", { name: name, email: email, password: password })

            if ((await signupRes).data.status == "success") {
                alert("signup success");

                const loginRes = axios.post("http://localhost:4000/user/login", { email: email, password: password })
